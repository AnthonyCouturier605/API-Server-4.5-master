...
module.exports =
    class collectionFilter {
            ...
            Object.keys(filterParams).forEach(function (paramName) {
                ...
                if (paramValue) {
                    switch (paramName) {
                        ...
                        case "keywordFields": instance.keywordFields = paramValue.split(','); break;
                        //case "keywords": instance.setKeywords(paramValue.split(','),instance.keywordFields); break;
                        case "keywords": instance.setKeywords(paramValue); break;
        ...
        // setKeywords(keywords,keywordFields){
        //     if (Array.isArray(keywords) && Array.isArray(keywordFields)) {
        //         for (let keyword of keywords) {
        //             for (let keywordField of keywordFields) {
        //                 this.addSearchKeyword(keywordField,`*` + keyword + `*`);
        //             }
        //         }
        //     }
        // }
        // addSearchKeyword(keyName, keyword) {
        //     if (this.model && !(keyName in this.model))
        //         return;
        //     this.keywords.push({ name: keyName, value: keyword });
        // }
        setKeywords(paramValue){
            this.keywords = paramValue.split(',')
        ...
        findByKeywords() {
            if (this.keywords.length > 0) {
                ...
                for (let item of this.filteredCollection) {
                    if (this.itemMatchKeywords(item))
        ...
        itemMatchKeywords(item) {
            if (item) {
                let matches =[];
                for (let keyword of this.keywords) {
                    for (let keywordField of this.keywordFields){
                        if (keywordField in item){
                            if (this.valueMatch(item[keywordField], keyword)){
                                    matches.push(true);
                                    break;
                            }
                        }
